...
public class ContratoPdfGen {
	...
	public void geraContrato(Contrato contrato, Testemunha test1, Testemunha test2) throws FileNotFoundException {
	...
	//	cal.setTime(contrato.getDataPagamento());
		...
		try {
			...
			cell = new Cell();
			p = new Paragraph("\n\n\n___________________________________________\nADMINISTRADOR: João Paulo Santos Teodoro\nCPF: 657.114.292-20");
			cell.add(p);
			cell.setTextAlignment(TextAlignment.LEFT);
			cell.setBorder(b2);
			table2.addCell(cell);
