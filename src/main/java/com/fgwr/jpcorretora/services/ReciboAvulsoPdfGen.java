...
public class ReciboAvulsoPdfGen {
	...
	public void geraRecibo(ReciboAvulso recibo) throws FileNotFoundException {

		Calendar cal = Calendar.getInstance();
		cal.setTime(recibo.getDataPgto());
		PdfWriter writer = new PdfWriter(FilesUtils.pathRecibosAvulso(recibo));
		...
		try {
			...
			cell = new Cell();
			Paragraph p = new Paragraph("RECIBO \nR$" + StringsUtils.formatarReal(recibo.getValorPago()));
			p.setFont(titleFont);
			p.setFontSize(16);
			...
			cell = new Cell();
			String nome = recibo.getPagador();
			Text t1 = new Text(nome).setBold();
			String cpf = StringsUtils.formatarCpfOuCnpj(recibo.getCpfPagador());
			...
			String valor = StringsUtils.formatarRealCifra(recibo.getValorPago());
			Text textValor = new Text(valor).setBold();
			p.add(" a importância de ");
			p.add(textValor);
			
			p.add(" (" + ValorExtenso.valorPorExtenso(recibo.getValorPago()) + ")"); 
			p.add(",");
			...
			cell = new Cell();
			p = new Paragraph("Vilhena, " + cal.get(Calendar.DAY_OF_MONTH) + " de " + new SimpleDateFormat("MMMMM", new Locale ("pt", "BR")).format(recibo.getDataPgto()) + " de " + cal.get(Calendar.YEAR) + ".");
			...
			p2.add(" (" + ValorExtenso.valorPorExtenso(recibo.getValorPago()) + ")"); 
			p2.add(",");
			
			p2.add(" Referente a " + recibo.getReferenteA());
			...
			cell2 = new Cell();
			p2 = new Paragraph("Vilhena, " + cal.get(Calendar.DAY_OF_MONTH) + " de " + new SimpleDateFormat("MMMMM", new Locale ("pt", "BR")).format(recibo.getDataPgto()) + " de " + cal.get(Calendar.YEAR) + ".");
